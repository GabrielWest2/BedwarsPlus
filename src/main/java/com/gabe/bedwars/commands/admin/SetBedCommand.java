...
public class SetBedCommand {
    public SetBedCommand(Player player, String[] args){
        if (args.length > 2) {
            if (Bedwars.getArenaManager().getArena(args[1]) != null) {
                ...
                if (!player.getTargetBlock(5).getType().toString().contains("_BED")) {
                    Bedwars.sendMessage(player, "You must target a bed block!");
                    return;
                }
                Location playerLoc = player.getTargetBlock(5).getLocation();
                team.setBed(playerLoc);
                Bedwars.sendMessage(player, player.getTargetBlock(5).getType() + " set bed");
                Bedwars.sendMessage(player, "Set team " + team.getColor() + team.getName() + "&e's bed to &6x: " + playerLoc.getBlockX() + " y: " + playerLoc.getBlockY() + " z: " + playerLoc.getBlockZ() + "&e.");
            } else {

                Bedwars.sendMessage(player, "Arena named &6" + args[1] + "&e doesn't exist.");
            }
        } else {
            Bedwars.sendMessage(player, "&cIncorrect Usage. Do /bwa setbed <name> <team>");
        }
    }
}
