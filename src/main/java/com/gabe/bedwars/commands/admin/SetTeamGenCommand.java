...
public class SetTeamGenCommand {
    public SetTeamGenCommand(Player player, String[] args) {
        if (args.length > 1) {
            if (Bedwars.getArenaManager().getArena(args[1]) != null) {
                ...
                Bedwars.sendMessage(player, "Set team " + team.getColor() + team.getName() + "&e's generator to &6x: " + playerLoc.getBlockX() + " y: " + playerLoc.getBlockY() + " z: " + playerLoc.getBlockZ() + "&e.");
            } else {

                Bedwars.sendMessage(player, "Arena named &6" + args[1] + "&e doesn't exist.");
            }
        } else {
            Bedwars.sendMessage(player, "&cIncorrect Usage. Do /bwa setteamgen <name> <team>");
        }
    }
}
